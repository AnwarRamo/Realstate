...
const PropertyList = () => {
  ...
  useEffect(() => {
    const fetchProperties = async () => {
      try {
        ...
        setSellProperties(sellResponse.data.properties || []);
      } catch (error) {
  ...
  const flatProperties = sellProperties.filter(property => property.propType === 'apartment');
  const houseProperties = sellProperties.filter(property => property.propType === 'house');
  const villaProperties = sellProperties.filter(property => property.propType === 'villa');
